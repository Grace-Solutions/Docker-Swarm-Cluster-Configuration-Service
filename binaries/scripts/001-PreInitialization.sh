...
NGINX_CONF_EOF
    ...
    # Create NginxUI reverse proxy config in sites-available (visible in NginxUI Sites section)
    # Then symlink to sites-enabled for activation
    local sites_available="${nginx_path}/sites-available"
    local sites_enabled="${nginx_path}/sites-enabled"
    local site_config_name="default-server.conf"
    local site_config="${sites_available}/${site_config_name}"

    # Remove old conf.d config if it exists (migration from old location)
    rm -f "${nginx_path}/conf.d/nginxui-management.conf" 2>/dev/null || true

    cat > "${site_config}" << 'NGINXUI_PROXY_EOF'
# Default Server - Management Interfaces
# Access NginxUI at http://<host>/nginxui/
# Access Portainer at http://<host>/portainer/
# Generated by dscotctl
# This file is managed in sites-available and visible in NginxUI Sites section
...
server {
    ...
    location / {
        return 404 "No site configured. Access NginxUI at /nginxui/ or Portainer at /portainer/\n";
        add_header Content-Type text/plain;
    }
}
NGINXUI_PROXY_EOF

    # Create TCP stream for Portainer agent tunnel (port 8000)
    local streams_available="${nginx_path}/streams-available"
    local streams_enabled="${nginx_path}/streams-enabled"
    local stream_config_name="portainer-agent-tunnel.conf"
    local stream_config="${streams_available}/${stream_config_name}"

    cat > "${stream_config}" << 'PORTAINER_STREAM_EOF'
# Portainer Agent Tunnel - TCP Stream
# Allows remote Portainer agents to connect on port 8000
# Generated by dscotctl

upstream portainer_agent_tunnel {
    server Portainer_Portainer:8000;
}

server {
    listen 8000;
    listen [::]:8000;
    proxy_pass portainer_agent_tunnel;
    proxy_connect_timeout 10s;
    proxy_timeout 300s;
}
PORTAINER_STREAM_EOF

    # Create symlink in streams-enabled if not exists
    local stream_symlink_path="${streams_enabled}/${stream_config_name}"
    if [ ! -L "${stream_symlink_path}" ]; then
        ln -sf "../streams-available/${stream_config_name}" "${stream_symlink_path}"
        echo "[PreInit] Created stream symlink: ${stream_symlink_path}"
    fi

    # Create symlink in sites-enabled if not exists
    local symlink_path="${sites_enabled}/${site_config_name}"
    if [ ! -L "${symlink_path}" ]; then
        ln -sf "../sites-available/${site_config_name}" "${symlink_path}"
        echo "[PreInit] Created symlink: ${symlink_path} -> ../sites-available/${site_config_name}"
    fi
